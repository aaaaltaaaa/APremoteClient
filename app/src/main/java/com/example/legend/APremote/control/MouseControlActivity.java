package com.example.legend.APremote.control;
    ...
    protected void initListener() {
        ...
        mLeftButton.setOnTouchListener((v, event) -> {
            if (event.getAction() == MotionEvent.ACTION_DOWN) {
                onLeftButton("down");
                mLeftButton.setBackgroundResource(R.drawable.button_down);
            }
            if (event.getAction() == MotionEvent.ACTION_UP) {
                onLeftButton("release");
                lbx = 0;
                lby = 0;
                mLeftButton.setBackgroundResource(R.drawable.button_up);
            }
            if (event.getAction() == MotionEvent.ACTION_MOVE) {
        ...
        mRightButton.setOnTouchListener((v, event) -> {
            if (event.getAction() == MotionEvent.ACTION_DOWN) {
                onRightButton("down");
                mRightButton.setBackgroundResource(R.drawable.button_down);
            }
            if (event.getAction() == MotionEvent.ACTION_UP) {
                onRightButton("release");
                mRightButton.setBackgroundResource(R.drawable.button_up);
            }
            return true;
        });
